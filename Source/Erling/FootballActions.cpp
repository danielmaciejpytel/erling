...
bool AFootballPlayer::StartAction(EAction Kind, const FString& Clip, float Rate, float StartFraction, bool Interruptible)
{
	...
	EntryVelocity = GetVelocity();
	EntryVelocity.Z = 0;
	ActionStartLocation = GetActorLocation();
	SetAnimation(Clip, false);
	PlaybackRate = Rate;
	AnimationTime = Sequence->GetPlayLength() * StartFraction;
	ActionStartTime = AnimationTime;
	ActionDuration = (Sequence->GetPlayLength() - AnimationTime) / Rate;
	ActionElapsed = 0;
	Action = Kind;
	ActionInterruptible = Interruptible;
	ActionAllowsMovement = false;
	PhysicalJump = false;
	ActionVelocity = FVector::ZeroVector;
...
{
	if (Action != EAction::None || PhysicalJump || TurningInPlace || GetCharacterMovement()->IsFalling())
		return;
	const FString Clip = Foot == TEXT("foot_l") ? TEXT("Kick_Left") : TEXT("Kick_Right");
	auto* Sequence = Animations.FindRef(Clip);
	if (!Sequence)
		return;
	BallTrapActive = true;
	BallTrapElapsed = 0.f;
...
void AFootballPlayer::CancelBallTrap()
{
	if (!BallTrapActive)
		return;
	BallTrapActive = false;
	BallTrapElapsed = 0.f;
	BallTrapFoot = NAME_None;
	PlaybackRate = 1.f;
	if (Action == EAction::None && !TurningInPlace)
		GetCharacterMovement()->bOrientRotationToMovement = true;
}
void AFootballPlayer::UpdateAction(float Dt)
{
	...
	{
		if (auto Sequence = Animations.FindRef(Name))
		{
			const float Length = Sequence->GetPlayLength();
			Time += Dt * Rate;
			Time = Loop ? FMath::Fmod(Time, FMath::Max(.01f, Length)) : FMath::Min(Time, Length);
		}
	};
	...
	if (Action != EAction::None)
	{
		...
		{
			if (Action == EAction::Fall)
			{
				StartAction(EAction::GetUp, TEXT("GetUp_Back"), 1.25f);
				return;
			}
			ClearAction();
		}
	...
	if (GetCharacterMovement()->IsFalling())
	{
		if (!PhysicalJump)
		{
			PhysicalJump = true;
			JumpElapsed = 0;
			JumpAnimationScale = 1.f;
			SetAnimation(Speed > 80 ? TEXT("Jump_Run") : TEXT("Jump"), false);
		}
		JumpElapsed += Dt;
		...
		if (auto Sequence = Animations.FindRef(CurrentAnimation))
		{
			...
			AnimationTime =
			    FMath::FInterpTo(AnimationTime, Desired, Dt, CurrentAnimation == TEXT("Jump_Run") ? 14.f * JumpAnimationScale : 12.f);
		}
		return;
	}
	if (PhysicalJump)
	{
		if (auto Sequence = Animations.FindRef(CurrentAnimation); Sequence && AnimationTime < Sequence->GetPlayLength() - .01f)
			return;
		PhysicalJump = false;
	}
	if (BallTrapActive)
	...
	if (Speed > 15)
	{
		...
		const FString Clip = ControlledWalk            ? TEXT("Walk")
		                     : Speed > SprintThreshold ? TEXT("Sprint")
		                     : Speed > RunThreshold    ? TEXT("Run")
		                                               : TEXT("Walk");
		SetAnimation(Clip);
...
void AFootballPlayer::StartPhysicalJump()
{
	...
	PhysicalJump = true;
	JumpElapsed = 0;
	PlaybackRate = GetVelocity().Size2D() > 80 ? 2.3f * JumpAnimationScale : 2.f;
	SetAnimation(GetVelocity().Size2D() > 80 ? TEXT("Jump_Run") : TEXT("Jump"), false);
	if (auto Sequence = Animations.FindRef(CurrentAnimation))
		AnimationTime = Sequence->GetPlayLength() * .28f;
	Jump();
...
void AFootballPlayer::BeginTurn(float TargetYaw)
{
	...
	SetAnimation(TEXT("Turn_Step"), false);
	PlaybackRate = 3.f;
	GetCharacterMovement()->bOrientRotationToMovement = false;
}
...
bool AFootballMode::LaunchShot(AFootballPlayer* P, const FVector& Velocity, bool CommittedShot)
{
	...
	if (auto PC = Cast<AFootballController>(P->GetController()))
...
void AFootballController::SlidePressed()
{
	...
	if (Screen != EScreen::Game || !Avatar || !Mode || !Avatar->CanAct() || Mode->HasBall(Avatar) || Avatar->GetVelocity().Size2D() < 200)
		return;
...
void AFootballController::OnGoal(AFootballPlayer* Scorer)
{
	if (Scorer != Avatar)
		return;
	GoalUntil = GetWorld()->GetTimeSeconds() + 5.f;
	GoalCelebrationUsed = false;
}
void AFootballController::BeginCelebration(bool Held)
{
	if (GoalCelebrationUsed || !Avatar)
	{
...
void AFootballController::UpdateActions(float Dt)
{
	...
	if ((ActiveScreen != EScreen::Game && ActiveScreen != EScreen::Main && ActiveScreen != EScreen::Credits) || !Avatar)
		return;
	...
	if (ShotBufferActive)
	{
		...
		else if (!PendingShot && Avatar->CanAct() && Now >= KickCooldown)
		{
			...
			bool RealContact = Mode->Possession->BallStopped;
			if (Avatar->GetMesh() && Avatar->GetMesh()->DoesSocketExist(TEXT("foot_l")) &&
			    Avatar->GetMesh()->DoesSocketExist(TEXT("foot_r")))
			{
				...
				RealContact = Gap.Size2D() <= 95.f && Gap.Z <= -25.f;
			if (RealContact)
			{
	...
	if (GoalSpacePending)
	{
		...
		if (GoalHoldRequested || !SpaceHeld)
	...
	if (PendingShot)
	{
		if (auto* Mode = GetWorld()->GetAuthGameMode<AFootballMode>(); Mode && Mode->Ball && !Mode->BallHidden)
		{
			...
			const FVector Contact = Avatar->GetActorLocation() + Avatar->GetActorForwardVector() * 82 +
			                        Avatar->GetActorRightVector() * Side - FVector(0, 0, 74);
			...
			if (Demo)
			{
				...
				const float ContactBlend =
				    FMath::SmoothStep(0.f, 1.f, FMath::Clamp(Avatar->ActionElapsed / FMath::Max(.01f, ShotContactTime), 0.f, 1.f));
			...
			else
			{
				...
				if (NormalKick)
				{
					...
					if (StrikeDirection.IsNearlyZero())
						StrikeDirection = Avatar->GetActorForwardVector().GetSafeNormal2D();
					...
					FVector Approach = DesiredActor - Avatar->GetActorLocation();
					Approach.Z = 0.f;
					...
					const FVector BallPlanarVelocity =
					    Mode->Ball->GetPhysicsLinearVelocity().GetSafeNormal2D() * Mode->Ball->GetPhysicsLinearVelocity().Size2D();
					...
					if (auto* Movement = Avatar->GetCharacterMovement())
					{
						...
						FVector CurrentPlanar = Movement->Velocity;
						CurrentPlanar.Z = 0.f;
					...
					if (Avatar->GetMesh())
					{
						...
						if (Avatar->GetMesh()->DoesSocketExist(FootName))
						{
							...
							if (!PhysicalShotContact && Avatar->ActionElapsed >= ShotContactTime)
							{
								...
								const bool LateNearContact = Avatar->ActionElapsed >= ShotContactTime + .12f && FootDistance <= 68.f &&
								                             FMath::Abs(FootToBall.Z) <= 75.f;
	...
	const bool DemoOrFlipFallback =
	    PendingShot &&
	    (ActiveScreen == EScreen::Main || ActiveScreen == EScreen::Credits || Avatar->Action == AFootballPlayer::EAction::Flip) &&
	...
	if (PendingShot && ActiveScreen == EScreen::Game && Avatar->Action == AFootballPlayer::EAction::Kick &&
	    Avatar->ActionElapsed >= ShotContactTime + .34f)
	{
...
void AFootballController::BallControlOn()
{
	if (Screen != EScreen::Game || !Avatar)
		return;
	auto* Mode = GetWorld()->GetAuthGameMode<AFootballMode>();
	if (!Mode || !Mode->HasBall(Avatar))
		return;
	BallControlHeld = true;
	if (!Sprint)
		Avatar->GetCharacterMovement()->MaxWalkSpeed = 190;
}
void AFootballController::BallControlOff()
{
	BallControlHeld = false;
	if (Avatar)
		Avatar->GetCharacterMovement()->MaxWalkSpeed = Sprint ? 765 : 500;
}
